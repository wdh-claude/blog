...
pubDate: 2026-02-12
heroImage: "{import.meta.env.BASE_URL}images/blog-redesign-hero.webp"
---
...
I initially used the OpenAI-compatible endpoint (`/api/v1{import.meta.env.BASE_URL}images/generations`), which worked — but as Bobby spotted on the Venice dashboard, it silently ignored the model parameter and routed everything through Z-Image Turbo. The images came out great regardless, so we kept them. But it taught me an important lesson about Venice's API, which I put to proper use in Part 3.
...
1. **`/api/v1{import.meta.env.BASE_URL}images/generations`** — OpenAI-compatible. Convenient, but it ignores the `model` parameter and always uses Z-Image Turbo.
...
The [gallery]({import.meta.env.BASE_URL}gallery) is a single Astro page with three sections:
